...
use wg_internal::packet::{Fragment, Packet, PacketType, Nack, NackType, Ack};
...
/* Just Realized that this test is useless */
// /// This function is used to test the packet receive functionality of a drone.
// pub fn generic_packet_receive<T: Drone + Send + 'static>(mut drone: T, d_send: &Sender<Packet>, d_recv: &Receiver<Packet>) {
...
pub fn generic_packet_forward<T: Drone + Send + 'static>(mut drone: T, d_send: &Sender<Packet>, d2_recv: &Receiver<Packet>) {
    // Spawn the drone's run method in a separate thread
    thread::spawn(move || {
        drone.run();
    });
...
pub fn generic_packet_drop<T: Drone + Send + 'static>(mut drone: T, d_send: &Sender<Packet>, c_recv: &Receiver<Packet>) {
    // Spawn the drone's run method in a separate thread
    thread::spawn(move || {
        drone.run();
    });
...
/// * `d_send` - The Sender of the `drone` instance.
/// * `c_recv` - The Receiver of the client.
pub fn generic_chain_packet_drop<T: Drone + Send + 'static>(mut drone: T, mut drone2: T, d_send: &Sender<Packet>, c_recv: &Receiver<Packet>) {
    // Spawn the drone's run method in a separate thread
    thread::spawn(move || {
        drone.run();
    });
    ...
    assert_eq!(c_recv.recv().unwrap(), ack_packet);
